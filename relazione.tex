...
Questi sviluppi hanno aumentato le preoccupazioni sulla solidità degli schemi crittografici attuali.
...
La crittografia a chiave pubblica costituisce una svolta fondamentale nel panorama della 
sicurezza informatica moderna. A differenza dei sistemi simmetrici, che vincolano mittente 
e destinatario alla condivisione di un unico segreto, i protocolli asimmetrici impiegano 
una coppia di chiavi: una pubblica, liberamente distribuibile, e una privata, 
custodita gelosamente dal proprietario.
...
\paragraph{Principi Matematici} 

Algebra modulare: ... continua da qui

\subsubsection{RSA} //todo
...
\paragraph{Principi matematici} Il funzionamento di RSA si basa su proprietà della teoria dei numeri e dell'algebra modulare. Ogni utente genera la propria coppia di chiavi attraverso i seguenti passaggi:
...
\paragraph{Sicurezza e dimensioni delle chiavi} La sicurezza di RSA dipende dall'impossibilità pratica di fattorizzare $n$ quando questo è sufficientemente grande. Conoscendo la fattorizzazione $n = pq$, un attaccante potrebbe infatti calcolare $\phi(n)$ e di conseguenza la chiave privata $d$.
...
\subsubsection{Vulnerabilità di RSA e ECC}
...
\subsubsection{Requisiti pratici e tempistiche}
...
\subsubsection{La risposta: crittografia post-quantistica}
...
Paper originale di Shor (1997): "Polynomial-Time Algorithms for Prime Factorization and Discrete Logarithms on a Quantum Computer"
