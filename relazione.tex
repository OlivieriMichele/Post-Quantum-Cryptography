...
\begin{enumerate}
    ...
    \item Obiettivo della "Forward Secrecy" e urgenza attuale
    Una definizione completa di PQC include la necessità di proteggere i dati non solo in futuro, ma anche oggi. Il modello di minaccia "Harvest Now, Decrypt Later" (raccogli ora, decifra dopo) suggerisce che attori malintenzionati possano archiviare dati criptati oggi per decifrarli quando saranno disponibili computer quantistici sufficientemente potenti. La migrazione alla crittografia post-quantistica è quindi considerata una priorità di sicurezza nazionale e infrastrutturale immediata.
\end{enumerate}
...
La PQC sposta la sicurezza dai problemi di fattorizzazione a nuove famiglie matematiche che, allo stato attuale della ricerca, non presentano vulnerabilità esponenziali quantistiche. Le fonti identificano i principali approcci di ricerca in:
• Basata su reticoli (Lattice-based): Considerata una delle più promettenti per prestazioni e sicurezza, include schemi come CRYSTALS-Kyber e CRYSTALS-Dilithium.
• Basata su codici (Code-based): Fondata sulla difficoltà di decodificare codici lineari casuali (es. l'algoritmo di McEliece).
• Basata su hash (Hash-based): Utilizza funzioni hash crittografiche per creare firme digitali (es. SPHINCS+).
• Multivariata: Basata sulla difficoltà di risolvere sistemi di equazioni multivariati.
• Basata su isogenie: Utilizza le proprietà dei grafi di isogenia tra curve ellittiche.
